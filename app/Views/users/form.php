...
<?= csrf_field() ?>
<div class="field"><label for="username">Username</label><input id="username" name="username" required maxlength="50" value="<?= esc(old('username') ?: ($user['username'] ?? ''), 'attr') ?>"></div>
<div class="field"><label for="full_name">Full name</label><input id="full_name" name="full_name" required maxlength="100" value="<?= esc(old('full_name') ?: ($user['full_name'] ?? ''), 'attr') ?>"></div>
